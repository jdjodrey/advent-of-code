    ...
    def drop(self, new_z):
        ...
        if self.is_vertical:
            ...
            if self.height != old_height:
                breakpoint()
                raise Exception("Height changed!")
        else:
            self.p1 = (self.p1[0], self.p1[1], new_z)
...
class Grid:
    ...
    def get_chain_reaction(self, target_label, falling=set()):
        brick = self.bricks_by_label[target_label]

        new_falling = set()
        for label in brick.bricks_above:
            b = self.bricks_by_label[label]

            if len(b.bricks_below - set(falling)) == 0:
                new_falling.add(label)

        if len(new_falling) != 0:
            for label in brick.bricks_above:
                falling.update(new_falling)

                falling = self.get_chain_reaction(label, falling)
...
def main():
    ...
    for b in grid.bricks:
        num_falling += len(grid.get_chain_reaction(b.label, falling=set([b.label]))) - 1

    print(f"Num falling bricks: {num_falling}")
